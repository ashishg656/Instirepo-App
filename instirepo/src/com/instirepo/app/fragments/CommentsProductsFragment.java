...
@SuppressLint("NewApi")
public class CommentsProductsFragment extends BaseFragment implements OnClickListener, ZUrls {
	...
	public static CommentsProductsFragment newInstance(Bundle b) {
		CommentsProductsFragment frg = new CommentsProductsFragment();
		frg.setArguments(b);
		return frg;
	}
	...
	public void onActivityCreated(Bundle savedInstanceState) {
		...
		productId = getArguments().getInt("postid");
	...
	private void loadData() {
		if (!isRequestRunning) {
			...
			final String url = getCommentsOnProduct + "?pagenumber=" + nextPage + "&product_id=" + productId;
			...
			ZApplication.getInstance().addToRequestQueue(req, getCommentsOnProduct);
		}
	}
	...
	protected void setAdapterData(CommentsListObject obj) {
		...
		if (adapter == null && obj.getComments().size() == 0) {
			showEmptyListView("No comments have been received on this product yet. Be the first one to comment", false,
					inflatedView);
		}
		...
		if (adapter == null) {
			...
			adapter = new CommentListAdapter(getActivity(), obj.getComments(), true);
			if (isMoreAllowed)
	...
	private void sendCommentToServer() {
		...
		StringRequest req = new StringRequest(Method.POST, addCommentOnProduct, new Listener<String>() {
		...
		ZApplication.getInstance().addToRequestQueue(req, addCommentOnProduct);
	}
