...
public class MyPostsFavouritePostsListAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder>
		implements AppConstants, ZUrls {
	...
	public MyPostsFavouritePostsListAdapter(Context context, List<PostListSinglePostObject> mData,
			boolean isMoreAllowed, boolean removePostFromListAfterRemovingFromFavourites) {
		super();
	...
	public void onBindViewHolder(ViewHolder holderCom, int pos) {
		...
		if (getItemViewType(pos) == Z_RECYCLER_VIEW_ITEM_NORMAL) {
			...
			if (obj.getImage() == null) {
				...
				ImageRequestManager.get(context).requestImage(context, holder.imagePost,
			...
			holder.numberOfPeopleViewed.setText(obj.getSeens() + " people viewed");
			holder.numberOfComments.setText("" + obj.getComment());
			holder.numberOfSaves.setText("" + obj.getSaves());
			holder.numberOfUpvotes.setText("" + (obj.getUpvotes() - obj.getDownvotes()));
			...
			GradientDrawable categoryBg = (GradientDrawable) holder.category.getBackground();
			int color = Color.parseColor(obj.getCategory_color());
			categoryBg.setStroke(context.getResources().getDimensionPixelSize(R.dimen.z_one_dp), color);
			...
			if (mData.size() == 1) {
				holder.externalPaddingLayout.setVisibility(View.VISIBLE);
				LinearLayout.LayoutParams params = (LinearLayout.LayoutParams) holder.externalPaddingLayout
						.getLayoutParams();
				params.height = (int) (context.getResources().getDisplayMetrics().heightPixels/1.5);
				holder.externalPaddingLayout.setLayoutParams(params);
			} else if (mData.size() == 2 && pos == 1) {
				holder.externalPaddingLayout.setVisibility(View.VISIBLE);
				LinearLayout.LayoutParams params = (LinearLayout.LayoutParams) holder.externalPaddingLayout
						.getLayoutParams();
				...
				holder.externalPaddingLayout.setVisibility(View.GONE);
			}
			...
			RecyclerView.LayoutParams p = (RecyclerView.LayoutParams) holder.emptyLayout.getLayoutParams();
	...
	public ViewHolder onCreateViewHolder(ViewGroup arg0, int type) {
		if (type == Z_RECYCLER_VIEW_ITEM_NORMAL) {
			View v = LayoutInflater.from(context).inflate(R.layout.post_by_teacher_list_item_layout, arg0, false);
			PostsHolderNormal holder = new PostsHolderNormal(v);
			return holder;
		} else if (type == Z_RECYCLER_VIEW_ITEM_LOADING) {
			View v = LayoutInflater.from(context).inflate(R.layout.loading_more, arg0, false);
			PostHolderLoading holder = new PostHolderLoading(v);
			return holder;
		} else if (type == Z_USER_PROFILE_ITEM_HEADER) {
			View v = LayoutInflater.from(context).inflate(R.layout.fake_header_user_profile, arg0, false);
			FakeHeaderHolder holder = new FakeHeaderHolder(v);
			return holder;
		} else if (type == Z_RECYCLER_VIEW_ITEM_NULL_CASE) {
			View v = LayoutInflater.from(context).inflate(R.layout.empty_view_layout, arg0, false);
	...
	class PostsHolderNormal extends RecyclerView.ViewHolder {
		...
		LinearLayout overflowIcon, externalPaddingLayout;
		FrameLayout seenByContainerLayout, openUserProfile, downvotePostContainer, savePostContainer;
		ImageView commentsLayout, savePostLayout, upvotePostLayout, downvotePostLayout;
		CircularImageView userImage;
		TextView userName, time, heading, description, numberOfPeopleViewed, numberOfUpvotes, numberOfSaves,
				numberOfComments;
		...
		public PostsHolderNormal(View v) {
			...
			seenByContainerLayout = (FrameLayout) v.findViewById(R.id.seenbycontainer);
			commentsLayout = (ImageView) v.findViewById(R.id.commentsviewconatiner);
			openUserProfile = (FrameLayout) v.findViewById(R.id.openuserprofilepost);
			...
			numberOfPeopleViewed = (TextView) v.findViewById(R.id.peopleviewednumebr);
			...
			upvotePostProgress = (ProgressBar) v.findViewById(R.id.upvotepostprogress);
			savePostProgress = (ProgressBar) v.findViewById(R.id.savepostprogress);
			postHeadingContainer = (FrameLayout) v.findViewById(R.id.postheadingdesccontainer);
			postImageContainer = (FrameLayout) v.findViewById(R.id.postpewimageviewcontainer);
			downvotePostLayout = (ImageView) v.findViewById(R.id.downvotepostimage);
			downvotePostProgress = (ProgressBar) v.findViewById(R.id.downvotepostprogress);
			downvotePostContainer = (FrameLayout) v.findViewById(R.id.downvotepostframe);
			savePostContainer = (FrameLayout) v.findViewById(R.id.savepostframeF);
			externalPaddingLayout = (LinearLayout) v.findViewById(R.id.hidelayoutonlytoshowexternalpadding);
		}
	}
	...
	class MyClickListener implements View.OnClickListener {
		...
		public void onClick(View v) {
			...
			case R.id.overflowiconpost:
				...
				showOverflowIconContent(mData.get(pos).getHeading(), mData.get(pos).getId(),
						mData.get(pos).isIs_saved(), pos, holder, mData.get(pos).isIs_following(),
						mData.get(pos).isIs_reported());
				break;
			...
			case R.id.openuserprofilepost:
				holder = (PostsHolderNormal) v.getTag();
				pos = holder.getAdapterPosition() - 1;
				openUserProfileFragment(mData.get(pos).getUser_id(), mData.get(pos).getUser_name(),
						mData.get(pos).getUser_image());
				break;
	...
	public void showOverflowIconContent(String message, final int postid, boolean isSaved, final int pos,
			final PostsHolderNormal holder, boolean isFollowing, boolean isReported) {
		BottomSheet sheet = new BottomSheet.Builder((BaseActivity) context).title(message)
				.sheet(R.menu.posts_sliding_panel_menu).listener(new DialogInterface.OnClickListener() {
					@Override
		...
		if (!isSaved) {
			...
			menu.getItem(1).setTitle(context.getResources().getString(R.string.menu_mark_important));
		} else {
			menu.getItem(1).setIcon(R.drawable.ic_star_pressed);
			menu.getItem(1).setTitle(context.getResources().getString(R.string.menu_marked_important));
		}

		if (!isFollowing) {
			menu.getItem(2).setIcon(R.drawable.ic_social_notifications_grey);
			menu.getItem(2).setTitle(context.getResources().getString(R.string.follow_post));
		} else {
			menu.getItem(2).setIcon(R.drawable.ic_social_notifications_blue);
			menu.getItem(2).setTitle(context.getResources().getString(R.string.following_post));
		}

		if (!isReported) {
			menu.getItem(0).setIcon(R.drawable.ic_flag_normal);
			menu.getItem(0).setTitle(context.getResources().getString(R.string.report_post));
		} else {
			menu.getItem(0).setIcon(R.drawable.ic_flag_selected);
			menu.getItem(0).setTitle(context.getResources().getString(R.string.reported_post));
		}
	}
	...
	protected void followPostRequest(final int postid, int pos) {
		...
		StringRequest req = new StringRequest(Method.POST, followPostRequest, new Listener<String>() {
			@Override
			...
			public void onErrorResponse(VolleyError arg0) {
				((BaseActivity) context).makeToast("Unable to follow/unfollow post. Check internet and try agin");

				mData.get(followPostPosition).setIs_following(!mData.get(followPostPosition).isIs_following());
			}
		}) {
			@Override
	...
	protected void reportPostRequest(final int postid, int pos) {
		...
		StringRequest req = new StringRequest(Method.POST, reportPostUrl, new Listener<String>() {
			@Override
			public void onResponse(String arg0) {
			...
			@Override
			public void onErrorResponse(VolleyError arg0) {
				((BaseActivity) context).makeToast("Unable to report/unreport post. Check internet and try agin");

				mData.get(reportPostPosition).setIs_reported(!mData.get(reportPostPosition).isIs_reported());
			}
		}) {
			@Override
			protected Map<String, String> getParams() throws AuthFailureError {
	...
	public void upvotePost(final int id, int pos, PostsHolderNormal holder, final boolean isUpvoteClicked) {
		if (!isUpotePostRequestRunning) {
			...
			StringRequest req = new StringRequest(Method.POST, upvotePost, new Listener<String>() {
				...
				public void onResponse(String arg0) {
					...
					mData.get(upvotePostPostition).setUpvotes(obj.getUpvotes());
					mData.get(upvotePostPostition).setDownvotes(obj.getDownvotes());
					mData.get(upvotePostPostition).setHas_downvoted(obj.isHas_downvoted());
					mData.get(upvotePostPostition).setHas_upvoted(obj.isHas_upvoted());

					if (upvotePostHolder != null) {
						upvotePostHolder.numberOfUpvotes.setText("" + (obj.getUpvotes() - obj.getDownvotes()));

						if (obj.isHas_upvoted()) {
							upvotePostHolder.upvotePostLayout.setSelected(true);
							upvotePostHolder.downvotePostLayout.setSelected(false);
						} else if (obj.isHas_downvoted()) {
							upvotePostHolder.upvotePostLayout.setSelected(false);
							upvotePostHolder.downvotePostLayout.setSelected(true);
						} else {
							upvotePostHolder.upvotePostLayout.setSelected(false);
							upvotePostHolder.downvotePostLayout.setSelected(false);
						}

						upvotePostHolder.upvotePostLayout.setVisibility(View.VISIBLE);
						upvotePostHolder.upvotePostProgress.setVisibility(View.GONE);
						upvotePostHolder.downvotePostLayout.setVisibility(View.VISIBLE);
						upvotePostHolder.downvotePostProgress.setVisibility(View.GONE);
					}

					if (obj.isHas_upvoted()) {
						((BaseActivity) context).showSnackBar("Successfully upvoted post");
					} else if (obj.isHas_downvoted()) {
						((BaseActivity) context).showSnackBar("Successfully downvoted post");
					} else {
						((BaseActivity) context).showSnackBar("Removed vote from post");
					}
				}
			}, new ErrorListener() {
				...
				public void onErrorResponse(VolleyError arg0) {
					isUpotePostRequestRunning = false;
					((BaseActivity) context).showSnackBar("Some error occured. Check internet connection");
	...
	public void markPostImportant(final int id, int pos, PostsHolderNormal holder) {
		if (!isMarkImpPostRequestRunning) {
			...
			StringRequest req = new StringRequest(Method.POST, markPostAsImportant, new Listener<String>() {

				@Override
				public void onResponse(String arg0) {
					isMarkImpPostRequestRunning = false;
					MarkPostImportantObject obj = new Gson().fromJson(arg0, MarkPostImportantObject.class);

					mData.get(markImportantPosition).setSaves(obj.getCount());
					mData.get(markImportantPosition).setIs_saved(obj.isIs_saved());

					if (markImportantHolder != null) {
						if (obj.isIs_saved()) {
							((BaseActivity) context).showSnackBar("Marked Post As Important");
							markImportantHolder.savePostLayout.setSelected(true);
						} else {
							((BaseActivity) context).showSnackBar("Removed Post From Important Posts List");
							markImportantHolder.savePostLayout.setSelected(false);
						}
						markImportantHolder.numberOfSaves.setText(obj.getCount() + "");

						markImportantHolder.savePostProgress.setVisibility(View.GONE);
						markImportantHolder.savePostLayout.setVisibility(View.VISIBLE);
					}

					if (removePostFromListAfterRemovingFromFavourites && !obj.isIs_saved()) {
	...
	void openUserProfileFragment(int id, String name, String image) {
		((UserProfileActivity) context).switchToUserProfileViewedByOtherFragment(id, name, image);
	}
}
