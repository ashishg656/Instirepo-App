...
public class BottomSheet extends Dialog implements DialogInterface {
	...
	BottomSheet(Context context, int theme) {
		...
		try {
			...
			mHeaderLayoutId = a
					.getResourceId(R.styleable.BottomSheet_bs_headerLayout,
							R.layout.bs_header);
			mListItemLayoutId = a.getResourceId(
					R.styleable.BottomSheet_bs_listItemLayout,
					R.layout.bs_list_entry);
			mGridItemLayoutId = a.getResourceId(
					R.styleable.BottomSheet_bs_gridItemLayout,
	...
	private void init(final Context context) {
		...
		LinearLayout mainLayout = (LinearLayout) mDialogView
				.findViewById(R.id.bs_main);
		...
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
			mDialogView.setPadding(0,
					location[0] == 0 ? helper.mStatusBarHeight : 0, 0, 0);
			mDialogView.getChildAt(0).setPadding(
					0,
					0,
					0,
					helper.mNavBarAvailable ? helper
		...
		if (getMenu().size() > limit) {
			...
			ActionMenuItem item = new ActionMenuItem(context, 0, R.id.bs_more,
					0, limit - 1, moreText);
			item.setIcon(more);
			menuItem.add(item);
			actions = menuItem;
		...
		BaseAdapter baseAdapter = new BaseAdapter() {
			...
			public View getView(int position, View convertView, ViewGroup parent) {
				...
				if (convertView == null) {
					...
					if (builder.grid)
						convertView = inflater.inflate(mGridItemLayoutId,
								parent, false);
					else
						convertView = inflater.inflate(mListItemLayoutId,
								parent, false);
				...
				if (item.getIcon() == null)
					holder.image.setVisibility(collapseListIcons ? View.GONE
							: View.INVISIBLE);
				else {
		...
		adapter = new SimpleSectionedGridAdapter(context, baseAdapter,
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				if (!((ActionMenuItem) adapter.getItem(position)).invoke()) {
					if (builder.menulistener != null)
						builder.menulistener.onMenuItemClick((MenuItem) adapter
								.getItem(position));
	...
	private void updateSection() {
		...
		if (!builder.grid && actions.size() > 0) {
			...
			if (sections.size() > 0) {
				SimpleSectionedGridAdapter.Section[] s = new SimpleSectionedGridAdapter.Section[sections
						.size()];
				sections.toArray(s);
	...
	/**
	 * If you make any changes to menu and try to apply it immediately to your
	 * bottomsheet, you should call this.
	 */
