...
public class PagerSlidingTabStrip extends HorizontalScrollView {
	...
	private static final int[] ATTRS = new int[] {
			...
			android.R.attr.textColor, android.R.attr.padding,
	...
	public PagerSlidingTabStrip(Context context, AttributeSet attrs,
		...
		paddingLeft = padding > 0 ? padding : a.getDimensionPixelSize(
				PADDING_LEFT_INDEX, 0);
		paddingRight = padding > 0 ? padding : a.getDimensionPixelSize(
		...
		isPaddingMiddle = a.getBoolean(
				...
				isPaddingMiddle);
		tabTypefaceStyle = a.getInt(
				R.styleable.PagerSlidingTabStrip_pstsTextStyle, Typeface.BOLD);
		tabTypefaceSelectedStyle = a.getInt(
				R.styleable.PagerSlidingTabStrip_pstsTextSelectedStyle,
				Typeface.BOLD);
		tabTextColorSelected = a
				.getColorStateList(R.styleable.PagerSlidingTabStrip_pstsTextColorSelected);
		textAlpha = a.getInt(R.styleable.PagerSlidingTabStrip_pstsTextAlpha,
				textAlpha);
		a.recycle();

		tabTextColor = colorStateList == null ? getColorStateList(Color.argb(
				textAlpha, Color.red(textPrimaryColor),
	...
	private void setMarginBottomTabContainer() {
		ViewGroup.MarginLayoutParams mlp = (MarginLayoutParams) tabsContainer
				.getLayoutParams();
		int bottomMargin = indicatorHeight >= underlineHeight ? indicatorHeight
				: underlineHeight;
		mlp.setMargins(mlp.leftMargin, mlp.topMargin, mlp.rightMargin,
				bottomMargin);
	...
	private void updateTabStyles() {
		for (int i = 0; i < tabCount; i++) {
			...
			v.setPadding(tabPadding, v.getPaddingTop(), tabPadding,
					v.getPaddingBottom());
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		if (tabsContainer.getChildCount() > 0) {
			tabsContainer.getChildAt(0).getViewTreeObserver()
	...
	private OnGlobalLayoutListener firstTabGlobalLayoutListener = new OnGlobalLayoutListener() {
		...
		public void onGlobalLayout() {
			...
			if (isPaddingMiddle) {
				...
				paddingLeft = paddingRight = getWidth() / 2
						- mHalfWidthFirstTab;
			}
			setPadding(paddingLeft, getPaddingTop(), paddingRight,
					getPaddingBottom());
			if (scrollOffset == 0)
	...
	protected void onDraw(Canvas canvas) {
		...
		Pair<Float, Float> lines = getIndicatorCoordinates();
		canvas.drawRect(lines.first + paddingLeft, height - indicatorHeight,
				lines.second + paddingLeft, height, rectPaint);
		// draw underline
		rectPaint.setColor(underlineColor);
		canvas.drawRect(paddingLeft, height - underlineHeight,
				tabsContainer.getWidth() + paddingRight, height, rectPaint);
		// draw divider
		if (dividerWidth != 0) {
			dividerPaint.setStrokeWidth(dividerWidth);
	...
	private class PageListener implements OnPageChangeListener {
		...
		public void onPageScrollStateChanged(int state) {
			...
			if (pager.getCurrentItem() - 1 >= 0) {
				View prevTab = tabsContainer
						.getChildAt(pager.getCurrentItem() - 1);
				notSelected(prevTab);
			}
			// Half transparent for next item
			if (pager.getCurrentItem() + 1 <= pager.getAdapter().getCount() - 1) {
				View nextTab = tabsContainer
						.getChildAt(pager.getCurrentItem() + 1);
				notSelected(nextTab);
			}
