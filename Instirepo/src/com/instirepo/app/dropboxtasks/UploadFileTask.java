 ...
 */
public class UploadFileTask extends AsyncTask<String, Void, Files.FileMetadata> {
	...
	public UploadFileTask(Context context, Files filesClient, Callback callback) {
		mContext = context;
	...
	protected Files.FileMetadata doInBackground(String... params) {
		...
		File localFile = UriHelpers
				.getFileForUri(mContext, Uri.parse(localUri));
			...
			try {
				...
				try {
					mFilesClient
							...
							.mode(Files.WriteMode.overwrite).run(inputStream);
				} finally {
					inputStream.close();
				}
